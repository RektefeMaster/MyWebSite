...
- **Hero 3D** (components/HeroScene.tsx): R3F sahnesi — tipografi (drei `<Text>`), stilize "M" + `MeshTransmissionMaterial` (cam/krom). Transmission buffer canlı: desktop `resolution=640` / `samples=10`, lite (mobil) `512` / `7`. Yüksek `samples` grenli/kumlu kırılmayı temizler; `resolution`'ı buffer bake sorunlarına karşı ~640 üstüne çıkarma. Lite DPR tavanı 1.5 / zemin 1.2 (`PerformanceMonitor` 1.0'a düşürmez). Env `frames={1}`. Offscreen / gizli sekmede `frameloop: never`. WebGL context loss’ta Canvas remount + hero görünür olunca invalidate pump (soft-nav dönüşünde boş kalmayı önler). `reactStrictMode: false` şart. Mesh mount'ta `useFrame` içinde easeOutCubic ölçek girişi yapar.
